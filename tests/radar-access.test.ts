...
describe("production radar access", () => {
  const env = { AGENT_INSTANCE_NAME: "production", RADAR_ACCESS_TOKEN: "radar-test", MANUAL_RUN_TOKEN: "legacy-test", JOB_LOBSTER_DB: { prepare: () => ({ bind: () => ({ all: async () => ({ results: [] }) }) }) } } as unknown as Env;
